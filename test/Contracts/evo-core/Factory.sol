...
contract Factory is IFactory {
    ...
    IEvoPriceOracle public evoPriceOracle;
    ...
    constructor(
        ...
        IEvoPriceOracle _evoPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        evoPriceOracle = _evoPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "Evo: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "Evo: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "Evo: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "Evo: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            evoPriceOracle.getPair(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "Evo: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "Evo: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "Evo: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "Evo: UNAUTHORIZED");
