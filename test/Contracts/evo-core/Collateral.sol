...
import "./interfaces/IEvoPriceOracle.sol";
import "./interfaces/IEvoCallee.sol";
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            IEvoPriceOracle(evoPriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Evo: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Evo: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Evo: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(shortfall > 0, "Evo: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        require(redeemAmount <= totalBalance, "Evo: INSUFFICIENT_CASH");

        // optimistically transfer funds
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            IEvoCallee(redeemer).evoRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Evo: INSUFFICIENT_REDEEM_TOKENS"
        );
