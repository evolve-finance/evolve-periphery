...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Evo: UNAUTHORIZED"); // sufficient check
        _setName(_name, _symbol);
        underlying = _underlying;
        borrowable0 = _borrowable0;
        borrowable1 = _borrowable1;
        evoPriceOracle = IFactory(factory).evoPriceOracle();
    }
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Evo: INVALID_SETTING");
        require(parameter <= max, "Evo: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Evo: UNAUTHORIZED");
    }
}
